...
public:
	...
	Bilet(const char* nume, const char* prenume, Meci meci, const char*zonaStadion):idBilet(idb)
	{
	...
	void setZonaStadionSiPretBilet(const char* zonaStadion)
	{
		...
		if (ok == true)
		{
			...
			throw"Prentru a seta un nou pret pe bilet, trebuie introdusa o zona noua care se regaseste in stadion";
	}
	//alte functii
	static void modificariBilet(Bilet& b, Meci meciuri[100], int ctMeciuri)
	{
		...
		while (buffer == false)
		{
			...
			cout << "Daca nu doresti sa efectuezi modificari, apasa 6" << endl;
			cout << "\nAlegerea ta este: ";
			cin >> index;
			while (cin.good() == 0)
			{
